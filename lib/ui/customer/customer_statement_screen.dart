...
class _CustomerStatementScreenState extends ConsumerState<CustomerStatementScreen> {
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      final db = ref.read(appDatabaseProvider);
      final currentBalance = await db.ledgerDao.getCustomerBalance(widget.customer.id);
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            _buildTableHeader(textColor, goldColor, bgColor),
            Expanded(
              child: _isLoading
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return _buildTransactionRow(
                              tx, runningBalance, textColor, subTextColor, isDark);
                          },
                        ),
            ),
            _buildFooter(runningBalance, textColor, goldColor, cardBg, isDark),
          ],
        ),
      ),
    );
  ...
  Widget _buildCustomerInfo(Color cardBg, Color textColor, Color subTextColor, Color goldColor) {
    return Container(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Text(
            isDebt ? '(مدين)' : '(دائن)',
            style: TextStyle(fontSize: 11, color: isDebt ? Colors.redAccent : Colors.green),
          ),
        ],
      ),
    );
  ...
  Widget _buildTableHeader(Color textColor, Color goldColor, Color bgColor) {
    return Container(
      margin: const EdgeInsets.fromLTRB(16, 8, 16, 0),
      ...
      child: Row(
        children: [
          const Expanded(flex: 2, child: Text('التاريخ', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFFC9A84C), fontSize: 12))),
          Expanded(flex: 3, child: Text('البيان', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFFC9A84C), fontSize: 12))),
          Expanded(flex: 2, child: Text('مدين (+)', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.redAccent, fontSize: 12))),
          Expanded(flex: 2, child: Text('دائن (-)', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.green, fontSize: 12))),
  ...
  Widget _buildTransactionRow(LedgerTransaction tx, double currentBalance, Color textColor, Color subTextColor, bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          bottom: BorderSide(color: const Color(0xFF30363D).withValues(alpha: 0.3), width: 0.5),
        ),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            ...
            child: Text(
              ...
              style: TextStyle(color: Colors.redAccent, fontWeight: tx.debit > 0 ? FontWeight.w600 : FontWeight.normal),
            ),
          ),
          Expanded(
            flex: 2,
            child: Text(
              ...
              style: TextStyle(color: Colors.green, fontWeight: tx.credit > 0 ? FontWeight.w600 : FontWeight.normal),
            ),
          ),
          Expanded(
            flex: 2,
            child: Text(
              ...
              style: TextStyle(
                fontWeight: FontWeight.bold,
                color: currentBalance > 0 ? Colors.redAccent : (currentBalance < 0 ? Colors.green : textColor),
              ),
            ),
          ),
        ],
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: (isDebt ? Colors.redAccent : Colors.green).withValues(alpha: 0.15),
