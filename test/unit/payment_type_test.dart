...
/// - day closed  -> DayClosedDialog gate (single authoritative day state);
/// - cash (نقدي) -> paid auto-syncs to the total; completing the invoice
///   persists a canonical 6-digit number with paid == total;
/// - credit (آجل) -> paid resets to 0.00, remaining shows the full total,
///   and completing without a customer is refused.
void main() {
  late AppDatabase db;
  ...
  group('Payment Type Selection Tests', () {
    ...
    testWidgets('cash selection auto-sets paid and completes canonically',
      ...
      // canonical number, fully paid cash sale. Walk-in cash writes no
      // customer ledger row by design (drawer accounting via journal), so
      // assert the journal sale entry keyed by the invoice id.
      ...
      final journal = await db.journalDao.getByPostingKey(
        'sale:INV${invoices.first.id}',
      );
      expect(journal, isNotNull);
      final lines = await db.journalDao.getLinesForEntry(journal!.id);
      expect(lines, isNotEmpty);
      expect(
        lines.fold<double>(0, (s, l) => s + l.debit),
        lines.fold<double>(0, (s, l) => s + l.credit),
      );
    });
